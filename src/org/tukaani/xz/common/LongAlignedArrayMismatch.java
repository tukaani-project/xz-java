 ...
 * to 8 byte, 4 byte or 2 byte offsets before deferring to another implementation.
 *
 * This is intended to be used in combination with an implementation which would
 * be expected to perform better if reads could be aligned to 8 byte offsets.
 * 
 * @author Brett Okken
 */
final class LongAlignedArrayMismatch implements ArrayMismatch {

    private final ArrayMismatch realMismatch;

    LongAlignedArrayMismatch(ArrayMismatch realMismatch) {
    ...
    public int mismatch(byte[] a, int aFromIndex, int bFromIndex, int length) throws Throwable {
        ...
        if (aFromAlignment == bFromAlignment) {
            ...
            // we could not get on 8 byte alignment, let's try 4 byte
            aFromAlignment = aFromIndex & 3;
            bFromAlignment = bFromIndex & 3;
