 ...
 * byteArrayViewVarHandle</a> for {@code long[]} using the
 * {@link ByteOrder#nativeOrder()} to process 8 bytes at a time.
 * 
 * The constructor will fail if {@code byteArrayViewVarHandle} cannot be invoked.
 * 
 * @author Brett Okken
 */
final class VarHandleLongArrayMismatch extends BaseLongArrayMismatch {

    /**
     * <p>
     * A jdk 9+ {@code byteArrayViewVarHandle} for {@code long[]} using
    ...
    static {
        ...
        try {
            ...
            long val = (long) getPrimitive.invokeExact(new byte[] { 0, 0, 0, 0, 0, 0, 0, 0}, 0);
            if (val != 0) {
                throw new IllegalStateException("invalid value: " + val);
            }
        } catch (Throwable t) {
            getPrimitive = null;
            final Logger logger = Logger.getLogger(VarHandleLongArrayMismatch.class.getName());
     ...
     */
    @Override
    long getLong(byte[] bytes, int index) throws Throwable {
        return (long) GET_PRIMITIVE.invokeExact(bytes, index);
    }
}
